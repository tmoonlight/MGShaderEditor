    ...
    public partial class MainForm : Form, IEffectCompilerOutput
    {
        ...
        private bool LoadEffectFile(string _strFileName)
        {
            try
            {
                ...
                Config.LastOpenendPath = _strFileName;
                Config.SaveChanges();
        ...
        void CreateUIParameters(string _effectSource)
        {
            //Clear old parameters
            m_game.Parameters.Clear();

            ParamType paramtype = ParamType.eUnknow;
            int idxStart = 0;
            int idxSource = 0;

            while (idxSource != -1)
            {
                paramtype = ParamType.eUnknow;

                //Get FloatParam
                idxStart = _effectSource.IndexOf("FloatParam", idxSource);
                if (idxStart != -1)
                {
                    paramtype = ParamType.eFloat;
                }

                //Get Texture2D
                if (paramtype == ParamType.eUnknow)
                {
                    idxStart = _effectSource.IndexOf("Texture2DParam", idxSource);
                    if (idxStart != -1)
                    {
                        paramtype = ParamType.Texture2D;
                    }
                }
                ...
                //End of line
                int idxEnd = _effectSource.IndexOf("\r", idxStart);
                if (idxEnd == -1)
                    break;
                ...
                //Get Line
                var line = _effectSource.Substring(idxStart, idxEnd - idxStart);
            ...
            //Update UI
            shaderParametersUserControl.DisplayParameters(m_game.Parameters);
        ...
        {
            ChangeRenderMode(Game1.RenderMode.Default);
        }

        private void _2DToolStripMenuItem1_Click(object sender, EventArgs e)
        {
            ChangeRenderMode(Game1.RenderMode.FullScreen);
        }
        ...
        {
            CusRec cr = new CusRec();
            if (cr.ShowDialog() == DialogResult.OK)
            {
                ChangeWindowSize(cr.ReturnValue.Width, cr.ReturnValue.Height);
            }
        }

        private void ChangeWindowSize(int width, int height)
        {
            this.m_game.ChangeWindowSize(width, height);
            Config.RecHeight = height;
            Config.RecWidth = width;
            Config.SaveChanges();
        }

        public void ChangeRenderMode(Game1.RenderMode rm)
        {
            this.m_game.ChangeRenderMode(rm);
            Config.RenderMode = rm;
            Config.SaveChanges();
        }
        ...
        {
            ChangeWindowSize(480, 320);
            //m_game.ChangeWindowSize(Config.RecWidth, Config.RecWidth);
            //m_game.ChangeRenderMode(Config.RenderMode);
        }
    }
